...
sed -i "s/^#LOCKD_TCPPORT=.*/LOCKD_TCPPORT=$LOCKD/" $CONFIG_PATH
sed -i "s/^#LOCKD_UDPPORT=.*/LOCKD_UDPPORT=$LOCKD/" $CONFIG_PATH
sed -i "s/^STATDARG=[^\n]*/STATD_PORT=$STATD\nSTATDARG=\'-p $STATD'/" $CONFIG_PATH
sed -i "s/^RPCMOUNTDOPTS=[^\n]*/MOUNTD_PORT=$MOUNTD\nRPCMOUNTDOPTS=\'-p $MOUNTD'/" $CONFIG_PATH
